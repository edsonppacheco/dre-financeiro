...
export async function GET(req: NextRequest) {
  try {
    ...
    const [rec, desp] = await Promise.all([
      supabase.from('planejamento_receitas').select('empresa_id, cliente_id, data_prevista, valor_previsto').in('empresa_id', empresaFiltro),
    ...
    const receitas = (rec.data ?? []).map((r) => ({ empresa_id: r.empresa_id as string, cliente_id: r.cliente_id as string, mes: mesDe(r.data_prevista as string), valor: Number(r.valor_previsto) }))
    ...
    for (const r of receitas) {
      ...
      porCliente[r.cliente_id] = round((porCliente[r.cliente_id] ?? 0) + v)
    }
    ...
    // Nomes de clientes e contas para as listas rápidas
    const clienteIds = Object.keys(porCliente)
    const contaIds = Object.keys(porConta)
    const [{ data: clientesRaw }, { data: planoRaw }] = await Promise.all([
    ...
    const clientes = Object.entries(porCliente)
      .map(([id, valor]) => ({ id, nome: nomeCliente[id] ?? '—', valor: round(valor) }))
